...
module params
  ...
  integer,parameter :: Nt = 3  ! number of t length+1
...
!  function jac(n1,n3,m1,m2) result()
!    integer,intent(in)::n1,n3,m1,m2
!    real(8) J(4,4)
!    J(1,1:4) =(/b*m1,1.0d0,-2.0d0*b*m1*x(0,n1,n3,m1,m2)+a*n1,-2.0d0*b*m2*x(0,n1,n3,m1,m2)+c1/)
!    J(2,1:4) =(/-b*m2,0.0d0,-2.0d0*b*m1*x(1,n1,n3,m1,m2)+c1,-2.0d0*b*m2*x(1,n1,n3,m1,m2)-a*n1/)
!    J(3,1:4) =(/1.0d0,-b*m1,-2.0d0*b*m1*x(2,n1,n3,m1,m2)-a*n3,-2.0d0*b*m2*x(2,n1,n3,m1,m2)-c1/)
!    J(4,1:4) =(/0.0d0,-b*m2,-2.0d0*b*m1*x(3,n1,n3,m1,m2)+c1,-2.0d0*b*m2*x(3,n1,n3,m1,m2)-a*n3/)
!     =  det(J);
!  end function jac
  function jac(n1,n3,m1,m2) result(g)
    use params
    integer,intent(in)::n1,n3,m1,m2
    real(8) g
    g = 2.0d0*b*c1**2*m2+a*c1*n1 + a*b**c1*m1**2*n1 +a*b**2*c1*m2**n1  &
         + a**2*b*m2*n1**2-a*c1*n3 -a*b**2*c1*m1**2*n3 -a*b**2*c1*m2**2*n3 &
         + a**2*b*m2*n3**2 - 2.0d0*b**2*c1*m2**2*x(4,n1,n3,m1,m2) -2.0d0*a*b**2*m1*m2*n1*x(4,n1,n3,m1,m2)&
         + 2.0d0*a*b**3*m1**2*m2*n3*x(4,n1,n3,m1,m2) +2.0d0*a*b**3*m2**3*n3*x(4,n1,n3,m1,m2) &
         + 2.0d0*b*c1*m2*x(1,n1,n3,m1,m2) - 2.0d0*b**2*c1*m1*m2*x(1,n1,n3,m1,m2)+ 2.0d0*a*b**2*m2**2*n1*x(1,n1,n3,m1,m2)&
         + 2.0d0*a*b*m1*n3*x(1,n1,n3,m1,m2) + 2.0d0*a*b**3*m1**3*n3*x(1,n1,n3,m1,m2) &
         +2.0d0*a*b**3*m1*m2**2*n3*x(1,n1,n3,m1,m2) & 
         + 2.0d0*b**2*c1*m2**2*x(2,n1,n3,m1,m2) + 2.0d0*a*b**3*m1**2*m2*n1*x(2,n1,n3,m1,m2)&
         + 2.0d0*a*b**3*m2**3*n1*x(2,n1,n3,m1,m2)&
         + 2.0d0*a*b**2*m1*m2*n3*x(2,n1,n3,m1,m2) -2.0d0*b*c1*m2*x(3,n1,n3,m1,m2) -2.0d0*b**2*c1*m1*m2*x(3,n1,n3,m1,m2) &
         - 2.0d0*a*b*m1*n1*x(3,n1,n3,m1,m2) -2.0d0*a*b**3*m1**3*n1*x(3,n1,n3,m1,m2) -2.0d0*a*b**3*m1*m2**2*n1*x(3,n1,n3,m1,m2) &
         + 2.0d0*a*b**2*m2**2*n3*x(3,n1,n3,m1,m2)
    g = abs(g)
  end function jac
! ----------------------------------------------------------------------------------------------------------
end module subprog
           ...
           do m1p = 1, Ns
              ...
              do m2p = 1, Ns
                 do mu = 1, Ndir
                    ...
                    d = sgm(mu,alpha,beta)*del(mu,n1-1,n1p-1,n3-1,n3p-1,m1,m1p,m2,m2p)*jac(n1-1,n3-1,m1,m2)
