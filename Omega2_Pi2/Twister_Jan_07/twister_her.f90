...
!           write(*,*) Dirac(i,j)
  ...
  do i = 1, Nint
     do j = 1, Nint
        Dirac_dag(i,j) = dconjg(Dirac(j,i)) ! take herimite conjugate of Dirac
     enddo
  enddo
  do i = 1, Nint
     do j = 1, Nint
        do k = 1, Nint
           Dirac(i,j) = Dirac(i,j) + Dirac(i,k)*Dirac_dag(k,j)
        enddo
        write(*,*) Dirac(i,j)
     enddo
  enddo
  deallocate(Dirac_dag)
