...
module params
  ...
  integer,parameter :: Nt = 8  ! number of t spacetime's length
  integer,parameter :: Ns = 8  ! number of s spacetime's length
  integer,parameter :: Nvol = Nt**2*Ns**2 ! numeber of lattice points
  integer,parameter :: Nint = Nvol*Nspn ! internal degree of freedom 
  real(8),parameter :: a = 1.d0 ! lattice spacing of t direction
  real(8),parameter :: b = 2.d0 ! lattice spacing of s direction
  real(8),parameter :: c1 = 1.0d-00  ! constant value of t2
  real(8),parameter :: c2 = 1.0d-00  ! constant value of t4 
...
130 format("Solving time [sec]=",f7.2)
  ...
  if(info == 0) then
     do j = 1, Nint
        write (10, *) atan(aimag(Eigen(j))/real(Eigen(j))),dsqrt(real(Eigen(j))**2+aimag(Eigen(j))**2),Eigen(j)
     enddo
     ifail = 0
  else
     Write (*, *)
